    ...
    function html5Upload() {
        ...
        sUploadURL= '/common/smarteditor/photo_uploader/popup/uploadfile.jsp'; 	//upload URL

        //파일을 하나씩 보내고, 결과를 받음.
        for(var j=0, k=0; j < nImageInfoCnt; j++) {            tempFile = htImageInfo['img'+j];
            try{
                if(!!tempFile){
    ...
    function onAjaxError (){
     ...
     function callFileUploader (){
         oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
        	 //sUrl  : location.href.replace(/\/[^\/]*$/, '') + '/mngr/common/imagePrevewUpload.do',
        	 sUrl  : '/common/imagePrevewUpload.do',
             //sCallback : location.href.replace(/\/[^\/]*$/, '') + '/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
        	 sCallback : '/common/smarteditor/photo_uploader/popup/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
